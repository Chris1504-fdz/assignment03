...
Outputs (saved to eval_outputs/<run_id>/):
  ...
  - human_win_rates.md + human_win_rates.csv (computed from labeled pairwise file, if provided)
  - adversarial_samples.md (qualitative failure-mode prompts for Part 4.2)

Notes:
  - Reward model scores: scalar reward per generated sample.
  - KL proxy: estimated on response tokens only (difference of token logprobs vs ref).
  - Human win-rate: requires labeling pairwise JSONL (winner field), then running --mode winrate.
"""
...
def generate_and_score(
    ...
    for bidx, start in enumerate(range(0, len(questions), EVAL_BATCH_SIZE), start=1):
        ...
        # KL proxy on response tokens only
...
# ----------------- Missing metrics helpers (human win-rate + adversarial prompts) -----------------

def compute_human_winrates(pairwise_rows: List[dict]) -> Dict[str, dict]:
    """
    Compute human win rate vs BASE from labeled pairwise rows.

    Accepted labels for row["winner"]:
      - "BASE"  -> model loss
      - row["model"] (e.g., "PPO") -> model win
      - "TIE"   -> tie
      - None/"SKIP"/"" -> ignored

    Returns per-model stats including win_rate over labeled examples.
    """
    stats = defaultdict(lambda: {"wins": 0, "losses": 0, "ties": 0, "labeled": 0, "total": 0})

    for r in pairwise_rows:
        model = r.get("model", "MODEL")
        base = r.get("base_model", "BASE")
        winner = r.get("winner", None)

        stats[model]["total"] += 1

        if winner is None or winner in ("", "None", "SKIP"):
            continue

        if winner == "TIE":
            stats[model]["ties"] += 1
            stats[model]["labeled"] += 1
        elif winner == model:
            stats[model]["wins"] += 1
            stats[model]["labeled"] += 1
        elif winner == base:
    ...
    for model, s in stats.items():
        denom = max(1, s["labeled"])
        out[model] = {
            **s,
            "win_rate": s["wins"] / denom,
            "loss_rate": s["losses"] / denom,
            "tie_rate": s["ties"] / denom,
        }
    return out

def write_human_winrate_tables(stats: Dict[str, dict], out_dir: str):
    md_lines = []
    md_lines.append("| Model | Labeled | Wins | Losses | Ties | Win rate |")
    md_lines.append("|---|---:|---:|---:|---:|---:|")
    for model in sorted(stats.keys()):
        s = stats[model]
        md_lines.append(
            f"| {model} | {s['labeled']} | {s['wins']} | {s['losses']} | {s['ties']} | {s['win_rate']:.3f} |"
        )
    md = "\n".join(md_lines) + "\n"
    write_text(os.path.join(out_dir, "human_win_rates.md"), md)

    header = ["Model", "TotalPairs", "Labeled", "Wins", "Losses", "Ties", "WinRate", "LossRate", "TieRate"]
    rows = []
    for model in sorted(stats.keys()):
        s = stats[model]
        rows.append([
            model, s["total"], s["labeled"], s["wins"], s["losses"], s["ties"],
            f"{s['win_rate']:.6f}", f"{s['loss_rate']:.6f}", f"{s['tie_rate']:.6f}",
        ])
    write_csv(os.path.join(out_dir, "human_win_rates.csv"), header, rows)

def adversarial_prompt_suite() -> Dict[str, List[str]]:
    """
    Part 4.2 failure-mode prompts:
      - excessive agreement / sycophancy
      - OOD prompts
      - capability loss checks

    These prompts are safe and focused on evaluation.
    """
    return {
        "Excessive agreement / sycophancy": [
        ...
        "Capability checks (possible regression)": [
...
@torch.no_grad()
def generate_only(run: ModelRun, prompts: List[str]) -> List[str]:
    run.model.eval()
    outs: List[str] = []

    for start in range(0, len(prompts), EVAL_BATCH_SIZE):
        batch_q = prompts[start:start + EVAL_BATCH_SIZE]
        batch_prompts = [format_hh_prompt(q) for q in batch_q]
...
def run_adversarial_eval(
    runs: List[ModelRun],
    reward_model: GPT2RewardModel,
    reward_tok: AutoTokenizer,
    out_dir: str,
):
    """
    Writes eval_outputs/<run_id>/adversarial_samples.md for Part 4.2.
    """
    suite = adversarial_prompt_suite()

    lines = []
    lines.append("# Adversarial / failure-mode qualitative prompts (Part 4.2)\n")
    lines.append(f"- device: `{DEVICE}`")
    lines.append(f"- generation: top_p={TOP_P}, temperature={TEMPERATURE}, max_prompt_len={MAX_PROMPT_LEN}, max_gen_len={MAX_GEN_LEN}\n")
    ...
    for category, prompts in suite.items():
        ...
        per_model = {}
        for run in runs:
            per_model[run.name] = generate_only(run, prompts)

        for j, q in enumerate(prompts):
            lines.append(f"### Prompt {j+1}")
            lines.append(q)
            lines.append("")

            for run in runs:
                comp = per_model[run.name][j]
                full_text = format_hh_prompt(q) + " " + comp
                ...
                r_attn = r_enc["attention_mask"].to(DEVICE)
                with torch.no_grad():
                    score = float(reward_model(r_input_ids, r_attn).item())

                lines.append(f"**[{run.name}]** (reward={score:.4f})")
                lines.append("```")
                lines.append(comp if comp else "(empty)")
                lines.append("```")
                lines.append("")

    write_text(os.path.join(out_dir, "adversarial_samples.md"), "\n".join(lines) + "\n")
...
def main():
    ...
    # Pairwise human judging file (for labeling)
    ...
    for i, q in enumerate(judge_questions):
        ...
        for run in runs:
            ...
            pairwise.append({
                ...
                "winner": None,  # fill in manually for human win-rate requirement
            })
    pairwise_path = os.path.join(out_dir, "pairwise_human_judge.jsonl")
    dump_jsonl(pairwise_path, pairwise)

    # Adversarial / failure-mode qualitative prompts (Part 4.2)
    vprint("\n[adv] Writing adversarial qualitative prompts (Part 4.2)...")
    run_adversarial_eval(runs, reward_model, reward_tok, out_dir)

    # If you already labeled a file, compute human win rates immediately
    labeled_path = os.path.join(out_dir, "pairwise_human_judge_labeled.jsonl")
    if os.path.exists(labeled_path):
        vprint("[winrate] Found labeled pairwise file; computing human win rates...")
        labeled_rows = load_jsonl(labeled_path)
        stats = compute_human_winrates(labeled_rows)
        write_human_winrate_tables(stats, out_dir)
    else:
        vprint("[winrate] No labeled file found yet (expected: pairwise_human_judge_labeled.jsonl).")
    ...
    vprint(f"  - adversarial_samples.md (Part 4.2 prompts)")
    vprint(f"  - human_win_rates.md / human_win_rates.csv (if labeled file provided)")
    vprint("=" * 80)
    print("\nMarkdown table:\n")
    print(summary_md)
...
def cli():
    """
    Modes:
      - eval (default): run evaluation and produce all artifacts.
      - winrate: compute human win rates from a labeled pairwise file.

    Usage:
      python src/eval_all_policies.py
      python src/eval_all_policies.py --mode winrate --pairwise_in eval_outputs/<run_id>/pairwise_human_judge_labeled.jsonl
    """
    p = argparse.ArgumentParser()
    p.add_argument("--mode", type=str, default="eval", choices=["eval", "winrate"])
    p.add_argument("--pairwise_in", type=str, default=None)
    args = p.parse_args()

    if args.mode == "winrate":
        if args.pairwise_in is None:
            raise ValueError("--pairwise_in is required for --mode winrate")
        out_dir = os.path.dirname(args.pairwise_in) or "."
        rows = load_jsonl(args.pairwise_in)
        stats = compute_human_winrates(rows)
        write_human_winrate_tables(stats, out_dir)
        vprint(f"[winrate] wrote human_win_rates.md and human_win_rates.csv to: {out_dir}")
        return
